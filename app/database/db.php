...
function selectAll($table, $conditions = [])
{
    ...
    if (empty($conditions)){
        ...
        $stmt = executQuery($sql, $conditions);
        $records = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
        return $records;
    }
}
...
function create($table, $data)
{
    ...
    //$sql = "INSERT INTO users SET usename=?, admin=?, email=?, password=?"
...
{
    global $conn;
    //$sql = "UPDATE users SET usename=?, admin=?, email=?, password=? WHERE id=?"
...
{
    global $conn;
    $sql = "DELETE $table FROM $table WHERE id=?";
